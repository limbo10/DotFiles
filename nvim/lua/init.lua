...
api.nvim_create_autocmd({ "BufNewFile", "BufRead", "BufEnter", "BufWritePre" }, {
...
keymap.set("i", "<C-f>", 'coc#float#has_scroll() ? "<c-r>=coc#float#scroll(1)<cr>" : "<Right>"', opts3)
keymap.set("i", "<C-b>", 'coc#float#has_scroll() ? "<c-r>=coc#float#scroll(0)<cr>" : "<Left>"', opts3)
...
local macro_content = ":s/[/{/g<CR>gv:s/]/}/g<CR>"

vim.keymap.set({ "n", "v" }, "<leader>z", function()
	vim.api.nvim_feedkeys(macro_content, "n", true)
end, { desc = "Execute original [] to {} macro" })
